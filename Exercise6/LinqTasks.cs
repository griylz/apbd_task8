...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.WithManagers();
            return result;
        }
        ...
        {
            int result = arr.GroupBy(i => i).Where(i=>i.Count()%2!=0).Select(i=>i.Key).First();
            //result=
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Where(d =>
                Emps.GroupBy(e => e.Deptno).Select(e => new { Deptno = e.Key, Count = e.Count() })
                    .Any(dc => dc.Deptno == d.Deptno && dc.Count == 5)||! Emps.GroupBy(e => e.Deptno).Select(e => new { Deptno = e.Key, Count = e.Count() }).Any(dc => dc.Deptno == d.Deptno))
                .OrderBy(d=>d.Dname);
            //result =
            return result;
        }
    ...
    {
        public static IEnumerable<Emp> WithManagers(this IEnumerable<Emp> emps)
        {
